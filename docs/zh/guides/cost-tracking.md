...
在 **模型 → 设置 → 计费模型来源** 中选择查价使用的模型 ID：

| 模式 | 查价范围 | 模型 ID |
|------|----------|---------|
| 重定向前 | 本次实际执行渠道的模型价格 | 客户端原始请求的模型 ID |
| 重定向后 | 本次实际执行渠道的模型价格 | 重定向后实际调用的模型 ID |

两种模式都使用本次请求实际产生的用量计算费用，无需配置独立的请求模型价格。模型编辑页的“模型卡片 → 成本”仅用于展示，不参与计费。

执行渠道中没有对应模型价格时，请求仍可继续并记录用量，费用显示为 `-`。不会借用其他渠道或其他模型的价格；明确配置为空的计费项目表示免费，费用为 `0`。

每次出站尝试都会保存渠道价格快照；重试换渠道或模型时重新取价，执行期间修改配置不会改变该次尝试的价格。实际渠道成本始终按实际模型计算。切换计费来源和修改价格都不会重算历史费用。
